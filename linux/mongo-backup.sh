...
usage () {
  ...
  echo " -D \$database_path : Optional [Only full restore]; Path to mongo database files."
  echo "                       default: '/data'"
  ...
  echo ""
  echo " Options:"
  echo "  -S \$volume_size  :  Optional [Only Full mode]; Specify the max size of the Virtual Volume."
  echo "                       Backup: used as Snapshot max. size"
  echo "                       Restore: used as volume size (same or bigger than the original one)"
  echo "                       default: '100G'"
  echo "  -G \$lvm_group    :  Optional; Set the LVM group where mongo data is/going to be"
  echo "                       default: 'mongo_data'"
  echo "  -V \$lvm_name     :  Optional [Only Full Backup]; Set the LVM Volume name where mongo data is/going to be"
  echo "                       default: 'mongodata'"
  echo "  -t \$last_op_time :  Optional; Set the path to the file where the last oplog time is/going to be"
